...
const Timer = ({ navigation }, props) => {
  ...
  const durationHoursRef = useRef("00");
  const durationMinutesRef = useRef("10");
  const durationSecondsRef = useRef("00");
  ...
  const intervalDurationHoursRef = useRef("00");
  const intervalDurationMinutesRef = useRef("01");
  const intervalDurationSecondsRef = useRef("00");
  const [intervalDurationHours, setIntervalDurationHours] = useState("00");
  const [intervalDurationMinutes, setIntervalDurationMinutes] = useState("01");
  const [intervalDurationSeconds, setIntervalDurationSeconds] = useState("00");
  ...
  useEffect(() => {
    ...
    if (isFocused){
      // Screen Keep Awake for Timer screen
  ...
  useEffect(() => {
    durationHoursRef.current = "00";
    durationMinutesRef.current = "10";
    durationSecondsRef.current = "00";
    intervalDurationHoursRef.current = "00";
    intervalDurationMinutesRef.current = "01";
    intervalDurationSecondsRef.current = "00";
    setIntervalDurationHours("00");
    setIntervalDurationMinutes("01");
    setIntervalDurationSeconds("00");
    setDurationHours("00");
  ...
  const discardSessionFunc = async () => {
  ...
  }}, [timerIsPlaying, timerCountDesc, appStateVisible]);

 /* useEffect(() => {
    if (Platform.OS === "android" && appStateVisible === "background") {
      var timerCountAsc = 0;
      // console.log(timerCountDesc);
      if (timerIsPlaying == true && !timerFinished) {
        if (timerCountDesc < 1) {
          setTimerIsPlaying(false);
          setTimerFinished(true);
          ...
          <View style={styles.modalInner}>
            <View style={styles.wheelEach}>
              ...
              <Picker
                ...
                selectedValue={durationHoursRef.current}
                pickerData={hoursList}
                onValueChange={(item) => (durationHoursRef.current = item)}
              />
              <Text style={{ color: "#000000", marginTop: 22 }}>Hours</Text>
            </View>
            <View style={styles.wheelEach}>
              {/* <WheelPickerExpo
                height={180}
              ...
              <Picker
                ...
                selectedValue={durationMinutesRef.current}
                pickerData={minutesList}
                onValueChange={(item) => (durationMinutesRef.current = item)}
              />
              <Text style={{ color: "#000000", marginTop: 22 }}>Minutes</Text>
            </View>
            <View style={styles.wheelEach}>
              {/* <WheelPickerExpo
                height={180}
              ...
              <Picker
                ...
                selectedValue={durationSecondsRef.current}
                pickerData={secondsList}
                onValueChange={(item) => (durationSecondsRef.current = item)}
              />
              <Text style={{ color: "#000000", marginTop: 22 }}>Seconds</Text>
            </View>
          </View>
        </Modal>
      </Portal>
  ...
  const bellPicker = (data) => (
    <View style={{ width: 130, alignItems:'center', minHeight: 150}}>
      <Image
  ...
  const onStartPress = () => {
    setStartEndFromDropdown("fromStart");
    setIntervalStartEndFrom("fromStart");
  };

  const onEndPress = () => {
    setStartEndFromDropdown("fromEnd");
    setIntervalStartEndFrom("fromEnd");
  };
  // From Start or From End
  const AddIntervalStartEndFromComponent = () => {
    const isStartSelected = startEndFromDropdown === 'fromStart';
    return (
      <View style={{flexDirection: 'row', }}>
        ...
        <View style={{flex:1, flexDirection: 'row', alignItems:'center'}}>
          <TouchableOpacity onPress={onStartPress} style={{flex:1,  marginLeft:15,marginRight:10,borderRadius:10,height: 45, backgroundColor: isStartSelected ? '#F0F0F0' : 'white', alignItems:'center', justifyContent:'center'}}>
            <Text style={{
              fontWeight: isStartSelected ? 'bold' : 'normal',
              color: 'grey',
              fontSize: 18
            }}>{'From Start'}</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={onEndPress} style={{flex:1,marginLeft:10,marginRight:15, height: 45,borderRadius:10, backgroundColor: !isStartSelected ? '#F0F0F0' : 'white', alignItems:'center', justifyContent:'center'}}>
            <Text style={{
              fontWeight: !isStartSelected ? 'bold' : 'normal',
              color: 'grey',
              fontSize: 18
       ...
       {/* <Picker
          ...
          selectedValue={startEndFromDropdown}
          pickerData={intervalStartEndFromList}
          onValueChange={(value) => {
            setStartEndFromDropdown(value), setIntervalStartEndFrom(value);
          }}
        />*/}
      ...
      <View style={styles.modalInner}>
        <View style={styles.wheelEach}>
          ...
          <Picker
            ...
            selectedValue={intervalDurationHoursRef.current}
            pickerData={hoursList}
            onValueChange={(item) => (intervalDurationHoursRef.current = item)}
          />
          <Text style={{ color: "#000000", marginTop: 22 }}>Hours</Text>
        </View>
        <View style={styles.wheelEach}>
          ...
          <Picker
            ...
            selectedValue={intervalDurationMinutesRef.current}
            pickerData={minutesList}
            onValueChange={(item) =>
              (intervalDurationMinutesRef.current = item)
            }
          />
          <Text style={{ color: "#000000", marginTop: 22 }}>Minutes</Text>
        </View>
        <View style={styles.wheelEach}>
          ...
          <Picker
            ...
            selectedValue={intervalDurationSecondsRef.current}
            pickerData={secondsList}
            onValueChange={(item) =>
              (intervalDurationSecondsRef.current = item)
            }
          />
          <Text style={{ color: "#000000", marginTop: 22 }}>Seconds</Text>
        </View>
  ...
  const existingIntervalEdit = () => {
    // console.log(intervalDurationMinutesRef.current);
  ...
  const showIntervalModal = (interval) => {
    ...
    let hours = interval?.intervalDurationHours.toString();
    let minutes = interval?.intervalDurationMinutes.toString();
    let second = interval?.intervalDurationSeconds.toString();
    intervalDurationHoursRef.current =
      interval?.intervalDurationHours < 10 ? `0${hours}` : hours;
    intervalDurationMinutesRef.current =
      interval?.intervalDurationMinutes < 10 ? `0${minutes}` : minutes;
    intervalDurationSecondsRef.current =
      interval?.intervalDurationSeconds < 10 ? `0${second}` : second;
    setStartEndFromDropdown(interval?.intervalStartEndFrom ?? "fromStart");
    setIntervalBellSound(interval.intervalBellSound);
    setVisibleEditIntervalSingle(true);
    setVisibleNewIntervalsMainModal(false);

    console.log(typeof intervalDurationMinutesRef.current.toString());
  };
      ...
      <ScrollView horizontal={false} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.container}>
          <View style={styles.containerInner}>
            ...
            {activePreset && newChanges && !visibleTimer && (
              <TouchableOpacity
                activeOpacity={0.8}
              ...
              <View>
                <View style={styles.modalInnerCenter}>
                  ...
                  <View style={{ width: "100%" }}>
                    {!timerFinished && Platform.OS === "ios" && <View style={{marginTop: 20}}>
                      <Text style={{fontSize: 18, textAlign:'center', fontStyle:'italic', fontWeight:'bold'}}>{'Keep your screen on while the timer is active.'}</Text>
                    </View>}
                    {!timerFinished && (
                      <Button
                        style={{
